# codeds-eval/python —— Python 工具链评测镜像
#
...
# 复现:生产应按 digest 钉死(FROM python:3.12-slim@sha256:...);离线无法核 digest,
#       这里先用固定 tag,TODO: 上线前换成 @sha256 摘要。
FROM python:3.12-slim

# 工具链依赖放在 COPY 之前,利用 layer 缓存。
# build-essential 给需要从源码编译的 wheel(C 扩展)兜底。
