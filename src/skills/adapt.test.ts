...
const DAO = new Set(["Read", "Write", "Edit", "Bash", "Grep", "AskUserQuestion"]);

describe("isForeignSkill — 无字典结构性检测", () => {
  it("CC 的 CamelCase 工具(反引号/工具语境)→ 外来", () => {
    expect(isForeignSkill("use the `WebFetch` tool then `MultiEdit`", DAO)).toBe(true);
  ...
  it("纯 dao 技能(dao 工具、无跨引用)→ 不算外来", () => {
    expect(isForeignSkill("先 `Read` 再 `Edit`,跑 `Bash` 验证", DAO)).toBe(false);
    expect(isForeignSkill("用 Read 工具读配置,再 Grep 工具搜符号", DAO)).toBe(false);
  });
