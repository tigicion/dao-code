...
## 先跑基线
读 DAO.md/README 拿构建测试命令;跑构建(失败=未完成)、跑测试套件(失败=未完成)、有 lint/类型检查就跑。收尾前调用 VerifyDone 逐条对证据核实;非琐碎改动再派 verify 子代理独立验证。
...
## 反自我合理化(出现就反着做)
"代码看起来对"→读不是验证,跑它;"我的测试过了"→你的测试也是 LLM 写的,独立另验;"大概没问题"→大概≠已验证;发现自己在写"为什么应该没问题"而不是发命令时→停,去跑那条命令;"环境没装验证要用的解释器/工具(没 python3、缺个库)"→先试装(apt-get/pip/npm 等)再跑,别退回"只读代码走查、不实际执行"——静态走查发现不了运行时才会暴露的问题,"装不上"和"没试着装"是两回事,后者不该被当成前者的理由跳过真实执行。
...
export interface BundledSkill {
  ...
  description: string;
  body: string;
  core?: boolean; // 核心:默认开、描述常驻上下文(可自动触发)。曾"不可关",现统一可在 /skills 关(参考 disableBundledSkills)
  modelInvokable?: boolean; // false=不让模型自动触发,只 /手动调(统一 disable-model-invocation)
  userInvocable?: boolean;  // false=不暴露 /手动调,只模型自动(统一 user-invocable:false)
}
