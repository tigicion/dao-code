...
export const TDD_BODY = `测试先行——【没有失败的测试,就不写实现】。新功能、改 bug、重构、改行为,都走红→绿→重构:

## 红:先写一个会失败的测试
1. 只测一个行为,名字写清"应该发生什么"。
2. 用真实代码,别 mock 掉被测对象本身(mock 只在不可避免时用)。
3. 写期望的 API/断言,即便它还不存在。

## 看它【真的失败】(不可跳过)
跑这个测试,确认:① 是 fail 不是 error(不是拼写/导入错);② 失败信息正是"功能缺失"那种。
- 测试一上来就过 → 你在测已有行为,改测试。
- 报 error → 先修 error,跑到它"正确地失败"为止。
没看着它失败,就不知道它测得对不对。

## 绿:写最小实现让它过
只写让这个测试通过的最少代码,不顺手加功能、不提前抽象(YAGNI)。跑测试确认:这个过了、其它没被弄坏、输出干净无警告。
- 测试还红 → 改实现,不是改测试。

## 重构:绿着清理
只在绿之后:去重、改名、抽 helper;保持全绿,不加新行为。

## 红旗(出现=停,删掉实现重来)
"先写实现回头补测试""手动验过了""这次太简单不用测""留着当参考、先补测试"。测试写在实现【之后】会立刻通过,而立刻通过证明不了任何东西。`;
...
export const BUNDLED_SKILLS: BundledSkill[] = [
  ...
  },
  {
    name: "debugging",
    description:
      "遇到 bug、报错、测试失败、构建/编译错、行为不对、跑不通、性能或集成问题,排查或动手改前用。",
    body: DEBUGGING_BODY,
    core: true,
  },
  {
    name: "tdd",
    description:
      "要写或改功能、加特性、修 bug、重构、改行为,动手写实现代码之前用。",
    body: TDD_BODY,
    core: true,
  },
  {
    name: "planning",
    description:
      "改动牵涉多文件、新依赖、架构/数据流、迁移,或路径不止一条等复杂改动,动手前用;小改不用。",
    body: PLANNING_BODY,
    core: true,
  },
  {
