...
export const notebookEditTool = defineTool({
  ...
  description:
    ...
    "cell_index 是【0-based】(和 Read 的 1-based 行号不是一回事,别混用);replace/delete 时下标越界会报错," +
    ...
    "也会补上空的 outputs/execution_count。整个 notebook 是普通 JSON 文件,但改前必须先用 Read 读过它——" +
  ...
  descriptionEn:
    ...
    "cell_index is [0-based] (unlike Read's 1-based line numbers — don't conflate the two); replace/delete error on an out-of-range index, " +
    ...
    "empty outputs/execution_count are also added. The notebook is a plain JSON file, but must be Read'd first before editing — " +
