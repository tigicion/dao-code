...
export const taskStopTool = defineTool({
  name: "task_stop",
  description:
    "停止一个任务:如果是后台子代理会被真正中止(发 abort 信号,不再消耗 token/时间);如果是 task_create 建的" +
    ...
    "就等于恢复原状了。停之前如果还有没读的中途输出,先用 task_get 看一眼再停,免得白白丢掉有价值的中间信息。" +
    "停止是终态,不能再重新启动同一个 id,要重跑只能重新调用 agent/task_create 建一个新任务。",
  descriptionEn:
    ...
    "(task_create) task, there's no real process to abort — it's just marked canceled. A no-op on already-ended tasks (naturally completed/failed/already " +
    ...
    "task_get before stopping so you don't lose valuable intermediate information for nothing. Stopping is terminal — you cannot restart the same id; to redo the " +
