...
export const taskListTool = defineTool({
  name: "TaskList",
  description:
    "列出任务(agent 后台派发的子代理 + TaskCreate 手动建的,同一套系统)。默认只列运行中的——这是你确认" +
    ...
    "TaskGet 精确查。图标含义:▶ running / ☑ completed / ✗ failed / ☐ canceled,扫一眼就知道整体情况,不用逐条读。" +
    "举例:之前派了三个后台子代理、记不清进度了,先无参数调一次这个,别凭印象假设它们都还在跑或都跑完了。" +
    "TaskCreate 手动建的任务和 agent(background:true)真派发的子代理在这里没有视觉区分,想知道某条具体是哪种," +
    "还是得靠描述内容自己判断或者用 TaskGet 看细节。",
  descriptionEn:
    "Lists tasks (subagents dispatched in the background via agent + ones manually created via TaskCreate — the same underlying system). By default only " +
    ...
    "once you have the id, use TaskGet for the full result/error/timestamps of a specific one. Icons: ▶ running / ☑ completed / ✗ failed / ☐ canceled — " +
    ...
    "don't assume they're all still running or all done. Manually created (TaskCreate) tasks and genuinely dispatched (agent background:true) subagents aren't " +
