...
export const monitorTool = defineTool({
  ...
  description: "起一个后台监控:跑一段脚本/命令,把它的 stdout 按行实时推送成通知(200ms 内的多行会合并成一条,避免刷屏)," +
    ...
    "'构建每完成一步就汇报'这类持续关注的场景;和 Bash 的 background 不一样——Bash 要你自己用 " +
    "BashOutput 主动去查,monitor 是它主动推给你。命令必须能自己退出(或用 persistent:true 让它跑满整个" +
    "会话,靠 TaskStop 手动结束)——写 tail -f/while true 这类不会自己退出的命令时,搭配 persistent:true;" +
    "只想要'某个条件满足时通知我一次'这种场景,让命令在条件满足后自己 exit,别用不会退出的命令加超时硬等。" +
    "默认 5 分钟超时(未设 persistent 时),到点自动终止并结算;persistent:true 时不设超时,靠 TaskStop 主动停。" +
    "危险命令(rm -rf、curl|sh 等)仍会被拦截强制确认,和 Bash 同一套规则。",
  descriptionEn: "Starts a background monitor: runs a script/command and pushes its stdout as real-time notifications, one line per event (lines " +
    ...
    "for 'tell me when an ERROR line appears' or 'report each build step' — unlike Bash's background mode where you have to actively poll via " +
    "BashOutput, monitor pushes to you instead. The command must exit on its own (or pass persistent:true to keep it running for the whole " +
    "session, stopped manually via TaskStop) — for commands that never exit on their own (tail -f, while true), pair them with persistent:true; " +
    ...
    "timeout, stop manually via TaskStop. Dangerous commands (rm -rf, curl|sh, etc.) are still forced to confirmation, same rules as Bash.",
  capability: "exec",
  approval: "required",
  shouldDefer: true,
  ...
  schema: z.object({
    ...
    persistent: z.boolean().optional().describe("true=跑满整个会话不设超时,靠 TaskStop 结束;默认 false"),
