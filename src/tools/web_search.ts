...
export const webSearchTool = defineTool({
  ...
  description: "用 DuckDuckGo 联网搜索,返回若干条结果(标题、URL、摘要),默认 5 条、最多 10 条(max_results 调)。" +
    ...
    "别直接报你训练数据里记得的旧版本信息,先搜一下确认现在真实的最新情况。当你不确定有哪些可用选项时" +
    "(替代工具、镜像源、库的用法等),也优先搜而不是凭记忆猜。DuckDuckGo 搜的是公开网页,搜不到" +
  ...
  descriptionEn: "Searches the web via DuckDuckGo, returning results (title, URL, snippet), 5 by default, up to 10 (adjustable via max_results). 30s timeout. " +
    ...
    "Also prefer searching when you're unsure what options exist (alternative tools, mirrors, library usage) rather than guessing from memory. " +
