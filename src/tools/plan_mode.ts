...
export const enterPlanModeTool = defineTool({
  ...
  description:
    ...
    "适用于新功能、多种可行方案、架构决策、多文件改动等场景。进入后只能用只读工具(read_file/grep_files/list_dir等)," +
    "写/执行类工具不可用。设计好方案后用 exit_plan_mode 退出并请求审批。",
  descriptionEn:
    "Enter plan mode (read-only). Use before writing code to explore the codebase, design an approach, and get user approval. " +
    "In plan mode, write/exec tools are unavailable; only read-only tools work. After designing, use exit_plan_mode to exit and request approval.",
  capability: "plan",
  approval: "auto",
  shouldDefer: true,
  schema: z.object({}),
  handler: async (_args, ctx) => {
    ...
    ctx.setMode("plan");
    return "已进入规划模式(只读)。探索代码库、设计方案,完成后用 exit_plan_mode 退出。";
  },
});
...
export const exitPlanModeTool = defineTool({
  ...
  schema: z.object({
    allowedPrompts: z.array(z.object({
      tool: z.enum(["exec_shell"]).describe("要预授权的工具(目前只支持 exec_shell)"),
