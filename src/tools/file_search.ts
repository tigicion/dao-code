...
export const fileSearchTool = defineTool({
  ...
  description: "在工作区内按文件名/路径 glob 搜索文件(如 *.ts 只匹配文件名、**/*.test.ts 能跨目录匹配完整相对路径)," +
    ...
    "按文件名/路径找文件优先用本工具,不要用 exec_shell 拼 find——不占审批。这是纯按名字/路径匹配,不看文件内容," +
    "按内容找用 grep_files。\n" +
    "边界:最多返回 100 个结果(比 grep_files 的 200 更少,大仓库模糊 glob 容易一下命中很多,想要更精确用更窄的 glob" +
    "或配合 path);查无匹配就是没有,不会像 grep_files 那样回显搜索范围,自己确认 glob 写对了。举例:想找刚编辑过的那个" +
  ...
  descriptionEn: "Searches files by name/path glob in the workspace (e.g., *.ts matches by filename only, **/*.test.ts matches the full relative path across directories), " +
    ...
    "Prefer this over shelling out to find via exec_shell — no approval needed. This matches by name/path only, not content — use grep_files for content search.\n" +
    "Boundaries: at most 100 results (fewer than grep_files's 200 — a loose glob on a large repo can match a lot; narrow the glob or add path for precision); " +
    "an empty match just means nothing found — unlike grep_files it doesn't echo back the search scope, so double-check the glob is correct. Example: you can't recall " +
