...
describe("TaskOutput", () => {
  it("增量返回自上次读取以来子代理任务新产生的消息", async () => {
    ...
    taskManager.appendMessage(agentId, {
      role: "assistant",
      content: null,
      tool_calls: [{ id: "c1", type: "function", function: { name: "Grep", arguments: '{"pattern":"foo"}' } }],
    });
    ...
    expect(third).not.toContain("Grep"); // 上一条工具调用已经在 first 里读过了
  });
