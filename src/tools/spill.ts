...
// 超大工具输出:把全量落盘到 .codeds/spill/,上下文里只放中间截断版 + 指针,
...
export function spillOutput(content: string, workspaceRoot: string, max = 16000): string {
  ...
  let pointer = "";
  try {
    const dir = path.join(workspaceRoot, ".codeds", "spill");
    mkdirSync(dir, { recursive: true });
    const name = `${Date.now()}-${Math.random().toString(36).slice(2, 6)}.txt`;
    writeFileSync(path.join(dir, name), content);
    pointer = `\n[完整输出 ${content.length} 字符已落盘:.codeds/spill/${name} —— 需要完整内容时用 read_file 读取(可配合 grep_files 精确定位)]`;
  } catch {
    /* 落盘失败 → 退化为纯截断 */
  }
