...
async function dispatchOne(
  ...
  try {
    ...
    // 工具返回了图片数据 → tool result 只含文字描述;图片 base64 存 imageData,
    // 由 loop.ts 在 tool messages 后面注入一条 user message 携带 image_url(千帆等 OpenAI 兼容 API 的 tool role 不接受 content 数组)。
    const imgData = ctx.currentImageData;
    ctx.currentImageData = undefined;
    audit(content);
    if (imgData) {
      return { role: "tool", tool_call_id: tc.id, content, imageData: imgData };
    }
