...
export const multiEditTool = defineTool({
  ...
  description:
    ...
    "第 N 处失败会报清楚是第几处、原文是什么,方便你调整;若是全角/半角标点、直弯引号、连字符/em dash 这类形近字符写岔了," +
    "会额外指出具体是第几个字符、两边分别是什么,别瞎猜,照着改。同一文件要做多处相关改动时,优先用这个而不是连发多个 Edit——" +
  ...
  descriptionEn:
    ...
    "a failure on the Nth edit reports which one and what text it was looking for, so you can adjust; if it's a look-alike-character slip (full-width vs half-width " +
    "punctuation, straight vs curly quotes, hyphen vs en/em dash), it'll also point out which character and what's on each side — fix that instead of guessing. " +
    "For multiple related changes to one file, prefer this over several " +
    ...
    return withFileLock(abs, async () => {
      ...
      for (let i = 0; i < args.edits.length; i++) {
        ...
        if (count === 0) {
          ...
          throw new Error(
            `第 ${i + 1} 处未找到 old_string(整体未改):${e.old_string.slice(0, 40)}${hint ? `。${hint}` : ""}`,
          );
        }
