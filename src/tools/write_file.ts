...
export const writeFileTool = defineTool({
  ...
  descriptionEn: "Creates a new file, or completely overwrites an existing one — content is the file's full text, replacing everything, no diff. " +
    ...
    "For partial changes, prefer Edit (precise replacement against the original, changes are reviewable, nothing unrelated gets touched) instead of retyping the whole " +
    ...
    "feedback (even a compile error counts), or use Edit for a targeted fix; don't discard a version you never ran just because you think the approach is wrong. " +
    ...
    "and getting cut off mid-write fails outright and wastes that entire generation. Split it: Write the skeleton or first part, then Edit to append/continue the rest.",
  capability: "write",
  approval: "required",
  schema: z.object({
  ...
  handler: async (args, ctx) => {
    ...
    return withFileLock(abs, async () => {
      ...
      // "思路不对"整篇重写,写下来的版本比只存在于推理里的完美设计更有价值。Edit
      ...
      if (exists && ctx.pendingUnverifiedWrites?.has(abs)) {
        throw new Error(
          ...
          `用 Edit 针对性修改不受此限制;如果确实需要先验证,现在就去跑一下上一版。`,
        );
      }
