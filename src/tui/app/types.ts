...
export interface AppDeps {
  ...
  // 取走并清空排队队列的全部内容:ESC 取消排队时用(丢弃返回值,不回填输入框);
  // 回合结束后仍有残留(极窄竞态窗口未被消费)时的兜底路径也用它取走内容开新回合补上。
  drainSteering?: () => string[];
  // 账户(profile)交互:/account 无参弹选择器(切换/增/删,空列表直接走粘贴引导)。省略则退回文本命令。
  ...
  listGatewayModels?: () => Promise<string[] | null>; // 当前账户是自定义网关时拉 /models 列表;非自定义网关返回 null
