...
export function dispatchCommand(input: string, session: Session): CommandResult {
  ...
  switch (cmd) {
    ...
    case "help":
      return {
        handled: true,
        output: "/mode <default|acceptEdits|plan|bypassPermissions> 权限模式 · /task 长任务自主 · /coordinator 协作编排 · /model 切模型 · /plan 切模式 · /yolo 自动批准 · /dod <命令> 验收命令 · /restore 回退检查点 · /theme 浅深色 · /clear 清空 · /compact 压缩 · /cost 用量 · /exit 退出",
      };
    case "exit":
    case "quit":
