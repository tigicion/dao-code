...
- **出错 / 调不通 / 行为异常** → 走根因优先的系统化调试:加载 debugging 技能,按四阶段(复现 → 定位 → 单一假设 → 改+验)来,别症状式乱改。
- **大型、多子系统、需分工** → 用 /coordinator(研究→综合→实现→验证);即便不开该模式,大任务也照这个分阶段:先(并行)调研、再综合出规格、再实现、最后验证,而不是边想边改混在一起。
