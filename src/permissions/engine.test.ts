...
describe("decide — 优先级:deny > 危险命令 > bypass > 敏感目标 > ask > allow > 模式/能力默认", () => {
  ...
  it("bypassPermissions:放行普通 exec;但危险命令仍 ask(S3.1 bypass-immune)", () => {
    expect(decide({ toolName: "Bash", argsJson: '{"command":"npm run test"}', capability: "exec", mode: "bypassPermissions", ...base })).toBe("allow");
    expect(decide({ toolName: "Bash", argsJson: rm, capability: "exec", mode: "bypassPermissions", ...base })).toBe("ask"); // rm -rf / 危险 → 即便 yolo 也要确认
  });
  it("ask 规则强制询问(优先于 allow)", () => {
    const rules = { ...emptyPermissions(), allow: ["Bash"], ask: ["Bash(rm:*)"] };
    expect(decide({ toolName: "Bash", argsJson: rm, capability: "exec", mode: "default", rules })).toBe("ask");
  });
  it("allow 规则命中放行", () => {
    const rules = { ...emptyPermissions(), allow: ["Bash(npm run test:*)"] };
    expect(decide({ toolName: "Bash", argsJson: '{"command":"npm run test"}', capability: "exec", mode: "default", rules })).toBe("allow");
  });
});
...
describe("decide — CC 1g:安全敏感目标", () => {
  ...
  it("bypass(yolo)下 WRITE_ONLY_SENSITIVE_TARGET(/etc、.git、shell启动脚本)不再 bypass-immune——用户已经显式yolo,读写都放行", () => {
    ...
    expect(decide({ toolName: "Edit", argsJson: '{"path":".git/config"}', capability: "write", mode: "bypassPermissions", ...base })).toBe("allow");
    expect(decide({ toolName: "Bash", argsJson: '{"command":"cat ~/.bashrc"}', capability: "exec", mode: "bypassPermissions", ...base })).toBe("allow");
    expect(decide({ toolName: "Bash", argsJson: '{"command":"cat /etc/postfix/main.cf"}', capability: "exec", mode: "bypassPermissions", ...base })).toBe("allow");
    expect(decide({ toolName: "Bash", argsJson: '{"command":"ls /etc/systemd/system/"}', capability: "exec", mode: "bypassPermissions", ...base })).toBe("allow");
    expect(decide({ toolName: "Write", argsJson: '{"path":"/etc/hosts"}', capability: "write", mode: "bypassPermissions", ...base })).toBe("allow");
  });
  it("yolo 下 shell 重定向截断(echo x > /etc/foo)仍要确认——这是独立的 S2.1 危险命令检测,不是 WRITE_ONLY_SENSITIVE_TARGET,不受这次放宽影响", () => {
    // `>` 重定向本身风险更高(shell 元字符面更大),跟"用 Write/Edit 工具结构化地
  ...
  it("显式 allow 规则可 opt-in 放行敏感目标", () => {
    ...
    expect(decide({ toolName: "Write", argsJson: '{"path":".ssh/config"}', capability: "write", mode: "bypassPermissions", rules: { ...emptyPermissions(), allow: ["Write"] } })).toBe("allow");
  });
  it("普通路径不受影响", () => {
    expect(decide({ toolName: "Write", argsJson: '{"path":"src/app.ts"}', capability: "write", mode: "bypassPermissions", ...base })).toBe("allow");
  });
});
...
describe("decide — auto 模式快速路径(分类器之前)", () => {
  ...
  it("③ 安全白名单工具 → 直接 allow", () => {
    expect(decide({ toolName: "TodoWrite", argsJson: "{}", capability: "write", mode: "auto", ...base })).toBe("allow");
  });
  it("Bash(危险)/ 敏感编辑 → 仍 ask(交分类器)", () => {
    expect(decide({ toolName: "Bash", argsJson: rm, capability: "exec", mode: "auto", ...base })).toBe("ask");
    expect(decide({ toolName: "Edit", argsJson: '{"path":"a/.ssh/id_rsa"}', capability: "write", mode: "auto", ...base })).toBe("ask");
  });
  it("网络查询(WebSearch/WebFetch)auto 下直接 allow(不再弹审批)", () => {
    expect(decide({ toolName: "WebSearch", argsJson: '{"query":"x"}', capability: "network", mode: "auto", ...base })).toBe("allow");
    expect(decide({ toolName: "WebFetch", argsJson: '{"url":"http://x"}', capability: "network", mode: "auto", ...base })).toBe("allow");
  });
  it("③' 只读 shell 命令(ls/cat/git status/管道)→ 直接 allow,不走分类器", () => {
    const ro = (cmd: string) => decide({ toolName: "Bash", argsJson: JSON.stringify({ command: cmd }), capability: "exec", mode: "auto", ...base });
  ...
  it("只读快速路径的安全边界:写/链式/重定向/危险/敏感 → 仍 ask", () => {
    const sh = (cmd: string) => decide({ toolName: "Bash", argsJson: JSON.stringify({ command: cmd }), capability: "exec", mode: "auto", ...base });
  ...
  it("显式 ask 规则命中时,即便命令只读也要问——用户显式规则优先于自动只读快速路径", () => {
    ...
    expect(decide({ toolName: "Bash", argsJson: '{"command":"ls /tmp"}', capability: "exec", mode: "default", rules })).toBe("ask");
    expect(decide({ toolName: "Bash", argsJson: '{"command":"ls /tmp"}', capability: "exec", mode: "auto", rules })).toBe("ask");
  });
});

describe("decide — 模式默认(无规则命中)", () => {
  it("default:read 自动放行,write/exec 询问", () => {
    expect(decide({ toolName: "Read", argsJson: '{"path":"a"}', capability: "read", mode: "default", ...base })).toBe("allow");
    expect(decide({ toolName: "Bash", argsJson: rm, capability: "exec", mode: "default", ...base })).toBe("ask");
...
describe("decideAsync(AST 路径,Bash 真实运行时走这条)— 只读快速路径同样生效", () => {
  ...
  it("显式 ask 规则优先于只读快速路径", async () => {
    const rules = { ...emptyPermissions(), ask: ["Bash(ls:*)"] };
    expect(await decideAsync({ toolName: "Bash", argsJson: '{"command":"ls /tmp"}', capability: "exec", mode: "default", rules })).toBe("ask");
  });
});
...
describe("isSensitiveCall", () => {
  it("写 .ssh / 命令含凭据路径 → 敏感;普通 → 否", () => {
    expect(isSensitiveCall("Write", '{"path":"a/.ssh/config"}')).toBe(true);
    expect(isSensitiveCall("Bash", '{"command":"cat ~/.aws/credentials"}')).toBe(true);
    expect(isSensitiveCall("Write", '{"path":"src/app.ts"}')).toBe(false);
  });
