...
- **额外护栏(实现加入,spec 原无)**:`background === true && (有 model 或 mode 覆盖)` -> 拒绝。后台子代理暂不支持调用级 model/mode 覆盖;若要换模型跑后台,为该用途定义一个带 model 的 agent_type 再用 background。后续版本可放开。
...
### A-6 嵌套禁止(实现收紧,原 spec 的"放开到 2 层"未采用)
- 实现选择:`agent.ts` 阈值保持 `>=1`(子代理内不可再派),且 `ALL_AGENT_DISALLOWED_TOOLS` 把 `agent` 工具对所有子代理全局禁用(双保险)。
- 原因:嵌套递归的成本/复杂度风险高于收益;子代理内需要协作的场景由主代理拆分后分别派发。
- 并发恒为 10(不做深度感知):无嵌套即无指数爆风险。
- 原始设计(放开到 2 层 + 深度感知并发)见版本历史,此处仅留痕。
