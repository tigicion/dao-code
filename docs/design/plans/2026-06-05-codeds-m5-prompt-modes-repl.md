...
- [ ] **Step 3: 写 `src/prompt/system_prompt.ts`**
  把 **设计文档 `docs/architecture/overview.md` §3 的 ```markdown 代码块正文**(从 `# 你是谁` 到 `# 回复风格` 整段,含其中的 `{model_id}` 与 `{project_instruction_files}` 占位符)**逐字复制**进一个 `const BODY` 模板字符串。然后在正文末尾**追加下面三段(EXACT)**,再写 `buildSystemPrompt`。最终文件结构如下(把 `__§3 正文逐字__` 替换为复制来的正文):
```ts
