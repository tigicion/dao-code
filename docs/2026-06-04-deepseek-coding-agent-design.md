...
## 7.5 技能(Skill)生态 —— 优势 d(前瞻设计,2026-06-07)

目标:开箱即用的 skill,与系统 prompt/工具/记忆形成生态。**核心原则:渐进式披露(progressive disclosure)——绝不把所有 skill 全文塞进 prompt**(会让前缀巨大且每请求重算,毁缓存)。三层加载:

| 层 | 内容 | 放哪(prompt 位置) | 缓存性质 |
|---|---|---|---|
| **L1 目录** | 每个 skill 的 `name + 何时用`(一行) | **稳定前缀**:系统 prompt 新增 `# 技能` 段,放在 `# 工具` 之后、`# 记忆` 之前 | ✅ 整会话不变 → 命中 |
| **L2 正文** | 被触发那个 skill 的完整指令(SKILL.md) | **调用时**作为一条消息**追加到对话尾部** | ✅ 只增长尾部,不破前缀 |
| **L3 资源/脚本** | skill 附带文件/脚本 | skill 内部按需走文件/exec 工具读取 | — |

系统 prompt 段落顺序(优势 c 缓存纪律:最稳在前、最易变在后):
```
13. # 环境
14. # 工具   {tools}     ← 能调什么工具
15. # 技能   {skills}    ← 新增:skill 目录(只放 名字+何时用)
16. # 记忆   {memory}
```
**机制与现有模式一致**:skill 目录 ↔ 工具清单(都是"我有哪些能力"的稳定清单,进前缀);用某 skill 时把正文当 tool 结果/消息追加尾部,从那轮起成为稳定历史、后续命中。系统 prompt 教"怎么用 skill"(选择/触发),skill 提供"做什么"。KPI = skill 触发 precision/recall + skill lift(有无该 skill 的解决率差)。

**Hooks(事件触发的外部命令,如 PreToolUse/SessionStart)与 prompt 的关系**:
- **hook 定义/配置不进 prompt**(纯运行时配置,模型看不到,harness 执行)→ 对前缀零影响。
- **hook 输出若要注入上下文,按时机分**:① SessionStart 类**一次性注入** → 启动定一次、整会话不变 → 可进前缀(同记忆注入);② **每轮触发、内容会变**(注入 git 状态/当前时间等)→ **必须追加到尾部**那条消息,绝不插进前缀或中间——否则即头号缓存反模式(每轮往前缀插变化内容,从注入点起整段作废)。
- **统一规则(与缓存纪律同源)**:静态目录/清单(skill 目录、一次性 hook 输出)进前缀;动态/按需内容(skill 正文、每轮 hook 输出)进尾部追加。如此加 skill/hooks 都不破坏已吃满的前缀缓存命中。
...
> **M7 已落地并实测(2026-06-06)**:`agent/compact`(estimateTokens 粗估、shouldCompact 阈值、compactMessages **按 user 消息切轮**保证 tool 序列完整)+ `/compact` 命令 + 轮后自动兜底(阈值 1M*0.85,罕触发)。摘要由独立一次 streamChat 生成(不带工具),作**第二条 system 消息**插在 messages[0](系统+记忆)之后。真网络实测:一轮含 20+ 工具消息的探索后 `/compact` 把对话 **27→6 条**,随后提问时模型据"早期对话摘要"答出被压缩轮里的事实(Alex/codeds)+ 保留近轮里的 vitest,跨压缩边界事实无损、无 API 报错。延后:单轮超长压缩(1M 下极难)、摘要可改用 flash 省钱、`/compact` 会 bust cache(§10,低频可接受)。
## 10. 最大化前缀 Cache(实现约束)
