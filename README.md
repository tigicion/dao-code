...
**A. 独立二进制(无需 Node,全平台)** —— 到 [Releases](../../releases) 下载对应平台:

| 平台 | 文件 | 运行 |
|---|---|---|
| macOS(Apple 芯)| `dao-darwin-arm64` | `chmod +x dao-darwin-arm64 && ./dao-darwin-arm64` |
| macOS(Intel)| `dao-darwin-x64` | 同上 |
| Linux | `dao-linux-arm64` / `dao-linux-x64` | `chmod +x dao-linux-* && ./dao-linux-*` |
| Windows | `dao-windows-x64.exe` | 双击或在终端运行 `dao-windows-x64.exe` |

> macOS 从**浏览器**下载若提示"无法验证开发者",先解除隔离:`xattr -d com.apple.quarantine dao-darwin-*`(或右键 →「打开」一次)。命令行(`curl`/`gh`)下载则无此限制。
> 可把文件改名为 `dao` 并放进 `PATH`,之后直接 `dao`。

**B. npm(需 Node ≥ 20,全平台)** —— 已发布:

```bash
npx dao-code        # 零安装试用(已实测可跑)
