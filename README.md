...
> **成本与体验兼得的终端编码 agent** —— 在最便宜的国产模型(DeepSeek V4)上,用「把上下文喂准 + 把缓存吃干」做出同级体验。墨黑·青玉·朱砂,太极开屏,《道德经》为伴。
>
...
一句话:**别人用更贵的模型换体验,DAO 用「上下文喂准 + 缓存吃干」在最便宜的模型上做出同级体验。**

### 💰 成本:结构性便宜,不是抠

- **国内可用、单价本就低** —— 选型 DeepSeek V4:国内直连、token 单价远低于第一梯队闭源模型。
- **极致吃缓存** —— DeepSeek 前缀缓存的命中价≈未命中的约 1/10。DAO 把系统前缀 / 工具表 / 记忆按「逐字节稳定」铺排,子代理换模型不破主缓存、记忆蒸馏对齐已缓存前缀——让本就便宜的缓存被持续命中、成本被进一步放大压低。`/cost` 实时看命中率。
  - **实测**(`npm run accept:cache` 可复现,多轮对话):缓存命中率随轮次 **31% → 89% → 94% → 96.3%** 爬升至稳态;同一会话里累计输入 token 从 1.4 万涨到 **32 万(23×)**,花费仅 **¥0.030 → ¥0.054**。未命中量全程恒定在约 1.2 万 tok(几乎全是第一轮一次性冷启动),上下文越长、新增部分越多落在缓存上、按约 1/10 单价计——几乎不要钱。真实编码每轮带大块文件/diff,会更快到高位。

### 🧠 体验:给模型需要的上下文 + 让模型适配你

体验拆开就两件事,下面这些功能都是它俩的不同侧面:

- **给模型上下文** —— 跨会话持久记忆、按需加载的 Skill、MCP 外部能力、自动压缩、(并行 / 后台)子代理。
- **让模型适配你** —— 记忆里的用户模型与偏好、Hooks 自定义生命周期行为、审批门「记住本会话 / 写 allow 规则」。
...
- **子代理** —— `agent` 工具把独立子任务派给子代理自主跑完,只回最终结果(支持并行 `tasks[]` 与后台 `background`)。
- **自动压缩** —— 接近上下文上限时自动把早期对话压缩成摘要,保留关键事实/改动/决定/未竟事项。
- **Skill / MCP / Hooks** —— 按需加载的开箱即用技能、外部 MCP 工具、生命周期钩子(详见 [扩展系统](#-扩展系统对标-claude-code))。

**🎐 体验 · 适配你 & 交互**

- **Ink 富终端 TUI** —— 基于 Ink(React for CLI)的 inline 渲染,保留终端原生滚动与文本选择;太极欢迎屏(程序化阴阳鱼、青玉→墨黑渐变词标、随机《道德经》名句)。对话展示对标 Claude Code:工具结果 `⎿` 子块(Bash/grep 显截断输出)、思考留历史(暗色 ✻ 块)、带行号+上下文+语法高亮的 diff、todo 复选框清单;spinner 用《道德经》《庄子》动词(悟/观/参/游/化…)随机轮换。默认截断、**Ctrl+O** 展开;`dao --verbose` 启动即全量。
